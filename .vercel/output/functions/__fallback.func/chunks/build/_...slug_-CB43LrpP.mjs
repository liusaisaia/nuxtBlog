import process from 'node:process';globalThis._importMeta_=globalThis._importMeta_||{url:"file:///_entry.js",env:process.env};import { defineComponent, withAsyncContext, computed, ref, mergeProps, unref, withCtx, createVNode, toDisplayString, toRaw, resolveComponent, defineAsyncComponent, h, getCurrentInstance, reactive, watch, Text, Comment, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderStyle, ssrRenderList, ssrInterpolate, ssrRenderComponent, ssrRenderAttr, ssrRenderClass, ssrRenderSlot } from 'vue/server-renderer';
import { a0 as pascalCase, a1 as kebabCase, G as destr } from '../nitro/nitro.mjs';
import { find, html } from 'property-information';
import { toHast } from 'minimark/hast';
import { c as useRoute, d as createError, u as useHead, b as useRuntimeConfig } from './server.mjs';
import { _ as __nuxt_component_0$1 } from './nuxt-link-CNNFjuF7.mjs';
import { CalendarDays, UserRound, Gauge } from 'lucide-vue-next';
...
_sfc_main$1.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("node_modules/.pnpm/@nuxt+content@3.11.2_@libsql+client@0.14.0_better-sqlite3@12.6.2_drizzle-orm@0.45.1_@libsql+c_4jtwky3j4faubt67okyhrpuize/node_modules/@nuxt/content/dist/runtime/components/ContentRenderer.vue");
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: allPosts } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "all-content-posts",
      () => queryCollection("content").order("date", "DESC").select("title", "path", "date", "tags").all()
    )), __temp = await __temp, __restore(), __temp);
    ...
    const relatedPosts = computed(() => {
      ...
      return (allPosts.value || []).filter((post) => post.path !== current.path).filter((post) => (post.tags || []).some((tag) => tags.has(tag))).slice(0, 4);
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<article${ssrRenderAttrs(mergeProps({ class: "layout-shell pb-20" }, _attrs))}><div class="fixed top-0 left-0 right-0 z-[60] h-[3px] bg-black/0"><div class="h-full bg-[var(--brand)] transition-[width] duration-150" style="${ssrRenderStyle({ width: unref(progress) + "%" })}"></div></div><header class="pt-8 md:pt-14"><div class="surface-strong rounded-3xl p-6 md:p-10 motion-rise"><div class="flex flex-wrap items-center gap-2 text-xs mb-4"><!--[-->`);
      ssrRenderList(unref(doc)?.tags || [], (tag) => {
        _push(`<span class="brand-chip rounded-full px-2.5 py-1">${ssrInterpolate(tag)}</span>`);
      });
      _push(`<!--]--><span class="text-mute">${ssrInterpolate(unref(doc)?.readingTime || "8 分钟阅读")}</span></div><h1 class="text-3xl md:text-5xl font-semibold tracking-tight leading-[1.1] max-w-4xl">${ssrInterpolate(unref(doc)?.title)}</h1><div class="mt-6 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-mute"><span class="inline-flex items-center gap-1.5">`);
      _push(ssrRenderComponent(unref(CalendarDays), {
        class: "w-4 h-4",
        "stroke-width": 1.9,
        "aria-hidden": "true"
      }, null, _parent));
      _push(` ${ssrInterpolate(formatDate(unref(doc)?.date))}</span><span class="inline-flex items-center gap-1.5">`);
      _push(ssrRenderComponent(unref(UserRound), {
        class: "w-4 h-4",
        "stroke-width": 1.9,
        "aria-hidden": "true"
      }, null, _parent));
      _push(` 作者：${ssrInterpolate(unref(doc)?.author || "Void Notes")}</span><span class="inline-flex items-center gap-1.5">`);
      _push(ssrRenderComponent(unref(Gauge), {
        class: "w-4 h-4",
        "stroke-width": 1.9,
        "aria-hidden": "true"
      }, null, _parent));
      _push(` 阅读进度 ${ssrInterpolate(Math.round(unref(progress)))}% </span></div></div></header><div class="mt-8 grid grid-cols-1 xl:grid-cols-12 2xl:grid-cols-14 gap-5 xl:gap-6"><aside class="2xl:col-span-3 hidden 2xl:block"><div class="surface rounded-2xl p-4 sticky top-24"><p class="text-xs uppercase tracking-[0.2em] text-mute mb-3">目录</p><nav class="space-y-1.5 max-h-[65vh] overflow-auto pr-1"><!--[-->`);
      ssrRenderList(unref(headings), (item) => {
        _push(`<a${ssrRenderAttr("href", `#${item.id}`)} class="${ssrRenderClass([unref(activeHeadingId) === item.id ? "brand-chip font-medium" : "text-soft hover:text-[var(--text)] hover:bg-[var(--brand-soft)]", "block px-3 py-2 rounded-lg text-sm transition-colors"])}">${ssrInterpolate(item.text)}</a>`);
      });
      _push(`<!--]--></nav></div></aside><div class="xl:col-span-8 2xl:col-span-7"><div class="surface-strong rounded-3xl p-6 md:p-8">`);
      _push(ssrRenderComponent(_component_ContentRenderer, {
        value: unref(docValue),
        class: "prose prose-lg article-prose max-w-none"
      }, null, _parent));
      _push(`</div></div><aside class="xl:col-span-4 2xl:col-span-4 space-y-4"><div class="surface rounded-2xl p-4"><p class="text-xs uppercase tracking-[0.2em] text-mute mb-3">同标签阅读</p><div class="space-y-2.5"><!--[-->`);
      ssrRenderList(unref(relatedPosts), (post) => {
        _push(ssrRenderComponent(_component_NuxtLink, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<h3 class="font-medium text-sm leading-snug"${_scopeId}>${ssrInterpolate(post.title)}</h3><p class="text-xs text-mute mt-1"${_scopeId}>${ssrInterpolate(formatDate(post.date))}</p>`);
            } else {
              return [
                createVNode("h3", { class: "font-medium text-sm leading-snug" }, toDisplayString(post.title), 1),
                createVNode("p", { class: "text-xs text-mute mt-1" }, toDisplayString(formatDate(post.date)), 1)
              ];
            }
          }),
          _: 2
      ...
      if (unref(relatedPosts).length === 0) {
        _push(`<p class="text-sm text-soft">暂无同标签文章。</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></div><div class="surface rounded-2xl p-4"><p class="text-xs uppercase tracking-[0.2em] text-mute mb-3">文章导航</p><div class="space-y-2.5 text-sm">`);
      if (unref(prev)) {
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: unref(prev).path,
          class: "block rounded-xl border border-soft p-3 hover:border-strong transition-colors"
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<p class="text-xs text-mute mb-1"${_scopeId}>上一篇</p><p${_scopeId}>${ssrInterpolate(unref(prev).title)}</p>`);
            } else {
              return [
                createVNode("p", { class: "text-xs text-mute mb-1" }, "上一篇"),
                createVNode("p", null, toDisplayString(unref(prev).title), 1)
              ];
            }
          }),
          _: 1
      ...
      if (unref(next)) {
        _push(ssrRenderComponent(_component_NuxtLink, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<p class="text-xs text-mute mb-1"${_scopeId}>下一篇</p><p${_scopeId}>${ssrInterpolate(unref(next).title)}</p>`);
            } else {
              return [
                createVNode("p", { class: "text-xs text-mute mb-1" }, "下一篇"),
                createVNode("p", null, toDisplayString(unref(next).title), 1)
              ];
            }
          }),
          _: 1
      ...
      _push(`</div></div></aside></div></article>`);
    };
  }
});
...
//# sourceMappingURL=_...slug_-CB43LrpP.mjs.map

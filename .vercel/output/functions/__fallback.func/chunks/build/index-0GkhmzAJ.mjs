...
import { defineComponent, withAsyncContext, computed, withCtx, createTextVNode, unref, toDisplayString, useSSRContext } from 'vue';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: postsData, refresh } = ([__temp, __restore] = withAsyncContext(() => useFetch(
      "/api/admin/posts",
      "$sdmAXgrsJg"
      /* nuxt-injected */
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(posts), (post) => {
        ...
        _push(`</td><td class="px-6 py-4"><span class="${ssrRenderClass([post.status === "published" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800", "px-2 py-1 rounded text-xs"])}">${ssrInterpolate(post.status === "published" ? "已发布" : "草稿")}</span></td><td class="px-6 py-4 text-gray-500">${ssrInterpolate(formatDate(post.createdAt))}</td><td class="px-6 py-4 text-right">`);
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: `/admin/posts/${post.id}`,
...
//# sourceMappingURL=index-0GkhmzAJ.mjs.map

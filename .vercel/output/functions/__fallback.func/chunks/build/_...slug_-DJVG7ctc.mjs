...
import { ssrRenderAttrs, ssrRenderStyle, ssrRenderList, ssrInterpolate, ssrRenderAttr, ssrRenderClass, ssrRenderComponent, ssrRenderSlot } from 'vue/server-renderer';
import { M as pascalCase, N as kebabCase, l as destr } from '../nitro/nitro.mjs';
import { find, html } from 'property-information';
import { toHast } from 'minimark/hast';
import { b as useRoute, d as createError, u as useHead, c as useRuntimeConfig } from './server.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<article${ssrRenderAttrs(mergeProps({ class: "container-page pb-20" }, _attrs))}><div class="fixed top-0 left-0 right-0 z-[60] h-[3px] bg-black/0"><div class="h-full bg-[var(--brand)] transition-[width] duration-150" style="${ssrRenderStyle({ width: unref(progress) + "%" })}"></div></div><header class="pt-8 md:pt-14"><div class="surface-strong rounded-3xl p-6 md:p-10 motion-rise"><div class="flex flex-wrap items-center gap-2 text-xs mb-4"><!--[-->`);
      ssrRenderList(unref(doc)?.tags || [], (tag) => {
        _push(`<span class="brand-chip rounded-full px-2.5 py-1">${ssrInterpolate(tag)}</span>`);
      });
      _push(`<!--]--><span class="text-mute">${ssrInterpolate(unref(doc)?.readingTime || "8 分钟阅读")}</span></div><h1 class="text-3xl md:text-5xl font-semibold tracking-tight leading-[1.1] max-w-4xl">${ssrInterpolate(unref(doc)?.title)}</h1><div class="mt-6 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-mute"><span>${ssrInterpolate(formatDate(unref(doc)?.date))}</span><span>作者：${ssrInterpolate(unref(doc)?.author || "Void Notes")}</span><span>阅读进度 ${ssrInterpolate(Math.round(unref(progress)))}%</span></div></div></header><div class="mt-8 grid grid-cols-1 xl:grid-cols-12 gap-5"><aside class="xl:col-span-3 hidden xl:block"><div class="surface rounded-2xl p-4 sticky top-24"><p class="text-xs uppercase tracking-[0.2em] text-mute mb-3">目录</p><nav class="space-y-1.5 max-h-[65vh] overflow-auto pr-1"><!--[-->`);
      ...
      _push(`<!--]--></nav></div></aside><div class="xl:col-span-6"><div class="surface-strong rounded-3xl p-6 md:p-8">`);
      _push(ssrRenderComponent(_component_ContentRenderer, {
        value: unref(docValue),
        class: "prose prose-lg max-w-none"
      }, null, _parent));
      _push(`</div></div><aside class="xl:col-span-3 space-y-4"><div class="surface rounded-2xl p-4"><p class="text-xs uppercase tracking-[0.2em] text-mute mb-3">同标签阅读</p><div class="space-y-2.5"><!--[-->`);
...
//# sourceMappingURL=_...slug_-DJVG7ctc.mjs.map
